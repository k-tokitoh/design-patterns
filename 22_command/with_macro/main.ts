...
class EditorClient {
  ...
  private creatingMacro: MacroCommand | undefined;

  // 文字コードを受け取って、登録されたフックを呼び出す
  input(value: string, editorTab: EditorTab): void {
    if (this.creatingMacro) {
      // マクロコマンド生成中の場合
      if (value === "$") {
        // マクロコマンドを発行する
        this.onCreate(this.creatingMacro);
        this.creatingMacro = undefined;
      } else {
        // マクロコマンドにコマンドを追加
        this.creatingMacro.append(this._input(value, editorTab));
      }
    } else {
      // マクロコマンド生成中でない場合
      if (value === "^") {
        // マクロコマンドの生成を開始
        this.creatingMacro = new MacroCommand();
        return;
      } else {
        // コマンドを発行する
        this.onCreate(this._input(value, editorTab));
      }
    }
  }

  // マクロ
  private _input(value: string, editorTab: EditorTab): Command {
    if (value === "\b") {
